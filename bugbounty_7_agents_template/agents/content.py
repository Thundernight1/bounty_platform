...
async def run(cfg: dict):
    ...
    if cfg.get("allowed_tests",{}).get("content_discovery", True) and os.system("which ffuf > /dev/null 2>&1") == 0 and os.path.exists("outputs/urls.txt"):
        ...
        if rc == 0:
            console.log("[green]content: ffuf çalıştı -> outputs/ffuf.json")
    else:
        ...
        console.log("[yellow]content: ffuf bulunamadı, robots.txt listesi oluşturuldu -> outputs/content_notes.txt")
