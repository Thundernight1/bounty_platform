...
async def main():
    ...
    ap.add_argument(
        "--auto-approve",
        action="store_true",
        help="Final raporu otomatik onayla (demo)",
    )
    args = ap.parse_args()
    ...
    if not args.auto_approve:
        console.print(
            "[yellow]outputs/REVIEW.md generated. "
            "Review and create 'outputs/APPROVED.txt' to approve."
        )
        ...
        with Progress(
            ...
            wait_task = progress.add_task(
                "[cyan]Waiting for approval...", total=60
            )
            for _ in range(60):
                if os.path.exists("outputs/APPROVED.txt"):
                    progress.update(wait_task, completed=60)
                    break
