...
async def _run_scans(job_id: str, request: JobRequest) -> None:
    ...
    if request.job_type == "attack_surface":
        result["web_scan"] = await asyncio.to_thread(run_zap_scan, request.target_url)  # type: ignore[arg-type]
        result["nuclei"] = await asyncio.to_thread(run_nuclei_scan, request.target_url)  # type: ignore[arg-type]
